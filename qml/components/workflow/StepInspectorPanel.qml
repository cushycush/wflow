...
// Slide-in inspector for the selected step.
Item {
    id: root
    ...
    // cond passed whole because partial replaces leave stale fields
    // when the kind switches.
    ...
    // Conditional-only; repeat ignores these.
    ...
    Rectangle {
        ...
        ScrollView {
            ...
            Column {
                ...
                Column {
                    ...
                    Row {
                        ...
                        SegmentedControl {
                            ...
                            onActivated: (v) => {
                                // Rebuild whole-cond so old fields don't linger.
                                let next
                    ...
                    // Env-only. Empty = any non-empty value of the var.
                    Rectangle {
                ...
                // Inner-step list for when/unless/repeat. Editing the
                // inner steps themselves is still via KDL.
                Column {
                    ...
                    // Leaf-only picker; nested containers would need a
                    // path-based selection model that doesn't exist yet.
                    Rectangle {
                        width: parent.width
                        height: 32
                        radius: Theme.radiusMd
                ...
                Column {
                    ...
                    // Lives on Step.note (not a separate Action::Note).
                    Column {
                        width: parent.width
                        spacing: 6
                        visible: root.sel != null
                        ...
                        Rectangle {
                            ...
                            TextArea {
                                ...
                                // Tells a "different card selected" sync
                                // apart from an "upstream note changed"
                                // sync; the latter respects focus so
