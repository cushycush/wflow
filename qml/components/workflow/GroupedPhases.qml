...
// Phase-grouped layout: setup / input / output buckets keyed off kind.
Column {
    id: root
    ...
    function phaseOf(kind) {
        if (["shell", "focus", "wait"].indexOf(kind) !== -1) return 0
        if (["key", "type", "click", "move", "scroll", "clipboard"].indexOf(kind) !== -1) return 1
        return 2
    }
