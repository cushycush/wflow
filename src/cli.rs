...
fn cmd_daemon() -> Result<ExitCode> {
    ...
    // there's nothing else to drive — Hyprland and Sway fire the
    // workflow in a separate `wflow run` subprocess, so the daemon
    // mostly sleeps.
...
fn collect_dispatchable_bindings(
    workflows: &[crate::actions::Workflow],
) -> Vec<crate::triggers::Binding> {
    use crate::actions::TriggerKind;
    let mut out = Vec::new();
    for wf in workflows {
        for t in &wf.triggers {
            if matches!(t.kind, TriggerKind::Chord { .. }) {
                out.push(crate::triggers::Binding {
                    workflow_id: wf.id.clone(),
                    workflow_title: wf.title.clone(),
                    trigger: t.clone(),
                });
...
fn same_chord(a: &crate::triggers::Binding, b: &crate::triggers::Binding) -> bool {
    use crate::actions::TriggerKind;
