...
/// Accepts the new `workflow "title" { ... }` and the legacy
/// `schema 1 / id / title / recipe { ... }` shape.
...
/// `id` stays empty here; the caller fills it from the filename.
fn decode_workflow_block(wf_node: &KdlNode) -> Result<Workflow> {
    ...
    for node in children.nodes() {
        match node.name().value() {
            ...
            // Timestamps live in the workflows.toml sidecar now; we
            // still read them here for legacy and pre-migration files.
...
fn decode_group(node: &KdlNode) -> Result<crate::actions::Group> {
    ...
    // First positional is the comment text (matches `note "text"`).
...
pub(super) fn decode_step(node: &KdlNode) -> Result<Step> {
    ...
    // Hard-error on typoed props (`retries=3` typoed as `retris=3`, etc).
    ...
    let action: Action = match name {
        ...
        "wait" => {
            // wait 500 | wait ms=500 | wait "1.5s". Pick one.
        ...
        "shell" => {
            ...
            // `with="..."` is preferred; `shell="..."` is the older alias.
        ...
        "use" => {
            // `use dev-setup` (bareword) and `use "dev-setup"` both work.
    ...
    // Reuse `_id` if present, else keep the fresh UUID from Step::new.
...
/// `chord "..."` ships in v0.4. `hotstring` and `when window-class=`
/// / `window-title=` parse today as forward-compat metadata.
...
/// Maps old verbs to their current name. The encoder only emits the
...
/// Per-action prop allowlist, keyed by canonical verb name. Plus
/// `COMMON_PROPS` on every action. Keep in lockstep with `decode_step`.
...
/// "Did you mean?" Returns the closest allowlisted name within
/// edit-distance 2.
