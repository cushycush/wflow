...
pub(super) fn cmd_path() -> Result<ExitCode> {
    // store::workflows_dir isn't pub yet, so we mirror it here.
...
pub(super) fn cmd_ids() -> Result<ExitCode> {
    // Stable contract: `id\ttitle\n`. Tabs in titles → spaces.
...
pub(super) fn cmd_list(as_json: bool) -> Result<ExitCode> {
    ...
    // Cap id at UUID length so long titles still get room.
    let id_w = workflows
        .iter()
