//! `wflow run` plus the daemon-internal `wflow trigger-fire` wrapper.
//! Path-or-id resolution, trust prompts, preflight, the live event
//! sink, and explain-mode subprocess rendering all live here.
...
pub(super) fn cmd_run(target: &str, dry_run: bool, explain: bool, yes: bool) -> Result<ExitCode> {
    ...
    // First-run trust check on workflows we didn't author. `--yes`
    // bypasses (cron/scripts).
    ...
    // Sink prints RunEvents as they land. `ran` counts StepDone events
    // (post-flatten, may exceed wf.steps.len() through `repeat`).
...
/// Daemon-side dispatch wrapper. Hyprland/Sway/portal binds exec this
/// instead of `wflow run` so we can gate on `trigger.when` against
/// the focused window. Case-insensitive substring match on class /
/// title. Failed probes fail open (KDE/GNOME portal users until per-DE
/// probes land). Predicate-skip exits 0 silently with one info trace.
pub(super) fn cmd_trigger_fire(target: &str) -> Result<ExitCode> {
    ...
    // We don't know which chord fired (daemon doesn't pass it back),
    // so: any unconditional chord → fire. All have `when` → fire if
    // any matches.
...
/// On-disk path for the trust hash. Path first, library id second.
...
/// Confirm-or-bail prompt for an untrusted workflow. Errors (not
/// denies) on no-TTY so non-interactive callers know to pass --yes.
...
/// Refuse to run if a needed host binary (notify-send, wl-copy) is
/// missing. wdotool-core is in-process; its failures land at first
/// dispatch, not here.
...
/// Collect external binaries needed by the workflow, recursing into
/// repeat / when / unless blocks.
...
/// Shell-quoted subprocess line(s) the engine would invoke for an
/// action. In-process actions render as `# comment-style` lines so
/// explain output stays one row per step.
...
/// POSIX-ish shell quoting: bare when safe, single-quoted otherwise
/// (embedded `'` becomes `'\''`).
