...
pub(super) fn cmd_new(title: &str, to_stdout: bool) -> Result<ExitCode> {
    ...
    // save() runs first so the file lands with the canonical filename,
    // then we overwrite it with the comment-bearing template.
...
pub(super) fn cmd_edit(target: &str) -> Result<ExitCode> {
    ...
    // sh -c so $EDITOR values like "nvim --noplugin" word-split.
