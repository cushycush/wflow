//! RecorderController. Wraps `recorder::Recorder` and pumps `RecFrame`
//! events onto the Qt thread.
...
pub mod qobject {
    ...
    unsafe extern "RustQt" {
        ...
        /// Returns the new workflow id, empty if there's nothing to save.
        #[qinvokable]
        ...
        let sink: recorder::FrameSink = Arc::new(move |frame: RecFrame| {
            ...
            match frame {
                ...
                RecFrame::StopRequested => {
                    // Esc came through the evdev stream. Run the same
                    // path the GUI Stop button takes.
