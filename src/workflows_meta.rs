//! `~/.config/wflow/workflows.toml`. Per-workflow timestamps and
//! card-position state, split out so a `.kdl` git diff is just steps,
//! not engine-bumped timestamps. Parse errors back the file up and
...
pub struct WorkflowMeta {
    ...
    /// Step-id → [x, y]. Set by the GUI canvas, restored on reopen.
...
/// Defaults on any failure. Broken files get backed up.
fn load_file() -> File {
    let path = match path() {
        Ok(p) => p,
        Err(e) => {
