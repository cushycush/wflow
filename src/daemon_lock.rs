//! Pidfile single-instance for `wflow daemon`. Lock is at
//! `$XDG_RUNTIME_DIR/wflow/daemon.pid`. Liveness via `/proc/$pid`.
//! TOCTOU between read and write is left unguarded: the loser will
//! bail when its chord-bind fails.
